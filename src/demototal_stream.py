...
#     # 配置参数（根据你的实际路径修改）
#     VIDEO_PATH = "/data1/lihenghao/code/VadCLIP/TestData/video/Abuse001_x264.mp4"                # 输入视频路径
...
#     # 获取用户输入的视频路径
#     while True:
#         VIDEO_PATH = input("请输入视频文件路径（例如：/data1/lihenghao/code/VadCLIP/TestData/video/Abuse001_x264.mp4）：").strip()
...
#             print(f"错误：请输入有效的视频文件（支持格式：{'、'.join(video_extensions)}）！")
#             continue
        
#         # 验证是否能打开视频
#         cap = cv2.VideoCapture(VIDEO_PATH)
#         if not cap.isOpened():
#             print(f"错误：无法打开视频文件，请检查文件是否损坏或格式不支持！")
#             cap.release()
#             continue
#         cap.release()
...
#     print(f"\n已选择视频：{os.path.basename(VIDEO_PATH)}")
...
#     print(f"  窗口长度：{WINDOW_SECONDS}秒")
#     print(f"  每个窗口最大抽帧数：{MAX_FRAMES_PER_WINDOW}帧")
#     print(f"  覆盖已有特征文件：{'是' if OVERWRITE else '否'}")
#     print(f"  检测设备：{DETECT_DEVICE}")
    
#     # 生成保存目录
#     video_name = os.path.splitext(os.path.basename(VIDEO_PATH))[0]
#     SAVE_DIR = "/data1/lihenghao/code/VadCLIP/TestData/video_window_features" + video_name + "_" + str(WINDOW_SECONDS) + "_" + str(MAX_FRAMES_PER_WINDOW)           # 特征保存目录
...
def save_single_window_features(features, window_idx, video_name, save_dir, config, overwrite=False):
    ...
    np.save(save_path, features)
    print(f"[保存] 滑动窗口{window_idx}特征已保存至：{save_path}")
    return save_path
...
def process_video_sliding_window(
    ...
    核心函数：滑动窗口处理视频流
    流程：
    1. 读取初始窗口（0~window_seconds秒）的帧 → 提取特征 → 检测 → 保存结果
    2. 滑动窗口：砍掉最前面step_seconds秒的帧 → 补充后面step_seconds秒的新帧
    3. 重复步骤1-2，直到视频结束
    """
    # 1. 初始化配置
    ...
    if frames_per_window >= config.max_frames_per_window:
        ...
        config.sample_rate_per_window = 1
    print(f"窗口帧数={frames_per_window}, 步长帧数={frames_per_step}, 采样率={config.sample_rate_per_window}")
    ...
    while current_frame_idx < total_frames:
        ...
        print(f"\n\n=== 处理滑动窗口{sliding_window_idx} [{window_start_time:.2f} - {window_end_time:.2f}秒] ===")
        print(f"[缓存状态] 当前窗口帧数量={len(window_frames_cache)}")
        
        # Step 2: 提取当前窗口特征
        print(f"[提取特征] 滑动窗口{sliding_window_idx}，帧数={len(window_frames_cache)}")
        window_features = extract_single_window_features(
        ...
        # Step 4: 执行暴力检测
        print(f"[暴力检测] 开始检测滑动窗口{sliding_window_idx}...")
        try:
            has_violence, violence_type, confidence = detect_violence(
                feat_save_path, violence_model_path, detect_device
            )
        except Exception as e:
            print(f"[检测错误] 滑动窗口{sliding_window_idx}检测失败：{str(e)}")
            has_violence = False
            violence_type = []
            confidence = 0.0
        
        # Step 5: 输出当前窗口检测结果
        print(f"\n[检测结果] 滑动窗口{sliding_window_idx}：")
        print(f"  是否存在暴力：{'是' if has_violence else '否'}")
        print(f"  检测置信度：{confidence:.4f}")
        if has_violence:
            print(f"  暴力类型：{', '.join(violence_type) if violence_type else '未明确'}")
    ...
    # 7. 输出最终汇总报告
    print(f"\n\n=== 所有滑动窗口处理完成 ===")
    print(f"总滑动窗口数：{sliding_window_idx}")
    print(f"检测到暴力的窗口数：{sum(1 for res in all_detection_results if res['has_violence'])}")
    ...
    for res in all_detection_results:
        ...
        violence_str = "有" if res['has_violence'] else "无"
        type_str = ', '.join(res['violence_type']) if res['violence_type'] else "无"
        print(f"滑动窗口{res['sliding_window_idx']} [{time_range}]：暴力={violence_str}，置信度={res['confidence']:.4f}，类型={type_str}")
...
if __name__ == "__main__":
    ...
    MAX_FRAMES_PER_WINDOW = 300                  # 每个窗口最大抽帧数
    SAVE_DIR = "/data1/lihenghao/code/VadCLIP/TestData/video_sliding_features" + os.path.basename(VIDEO_PATH).split(".")[0] + f"_{WINDOW_SECONDS}s_step{STEP_SECONDS}s"  # 特征保存目录
    OVERWRITE = True                             # 是否覆盖已有特征文件
    DETECT_DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
