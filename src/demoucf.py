...
def detect_violence(feat_path, model_path, device='cuda'):
    ...
    args, _ = ucf_option.parser.parse_known_args()
    ...
    label_map = {
        ...
        'Arson': 'Arson',  # 纵火
        'Assault': 'Assault',  # 袭击
        'Burglary': 'Burglary',  # 入室盗窃
        'Explosion': 'Explosion',  # 爆炸
        'Fighting': 'Fighting',  # 打架
        'RoadAccidents': 'RoadAccidents',  # 交通事故
        'Robbery': 'Robbery',  # 抢劫
        'Shooting': 'Shooting',  # 枪击
        'Shoplifting': 'Shoplifting',  # 入店行窃
        'Stealing': 'Stealing',  # 偷窃
        'Vandalism': 'Vandalism'  # 故意破坏
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("使用方法: python ucf_demo.py <特征文件路径> <模型权重路径>")
        sys.exit(1)
...
#  python src/demoucf.py /data1/lihenghao/data/UCFClipFeatures/Stealing/Stealing114_x264__5.npy /data1/lihenghao/code/VadCLIP/model/model_ucf.pth